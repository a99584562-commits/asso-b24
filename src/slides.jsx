...
function Docs() {
  return (
    <Frame>
      <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-2">
        <div>
          <Title eyebrow="Внутренние процессы · Документы" lead="ДДУ, договоры подряда, акты — формируются из шаблонов и согласуются по маршруту прямо в системе. Подпись — без бумаги, через КЭДО.">
            Документооборот <br /> и согласования
          </Title>
        </div>
        <div className="space-y-3">
          {[
            { t: "Шаблоны договоров", d: "ДДУ и подряд генерируются из полей сделки за секунды" },
            { t: "Маршрут согласования", d: "Юрист → финансы → директор, с контролем сроков" },
            { t: "КЭДО без бумаги", d: "Электронная подпись сотрудников и контрагентов" },
            { t: "Единый архив", d: "Все документы привязаны к объекту и сделке" },
          ].map((x, i) => (
            <Reveal key={x.t} i={3 + i}>
...
function Showcase() {
  return (
    <Frame>
      <Title eyebrow="Сверх коробки · Вайбкод" lead="Битрикс24 — гибкая платформа. То, чего нет в коробке, мы дорабатываем под АССО-Строй. Вот примеры решений, которые мы делаем быстро.">
